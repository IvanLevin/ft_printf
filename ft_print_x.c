...
static int	zeros_amount_x(t_lists *list, int len)
{
	...
	{
		if (list->width > list->precision)
			return (list->sharp != 1 ? list->precision - len : list->precision -
					len - list->sharp);
		else
...
static int	space_amount_x(char *str, t_lists *list, int len)
{
    if (!ft_strcmp(str, "0"))
        return (list->width);
	...
	if (len > 0 && list->width > len && list->width < list->precision &&
		...
		if (list->width < list->precision)
			return (0);
	if (list->width > len && len < list->precision && list->sharp)
		return (list->sharp == 1 ? list->width - list->precision : list->width -
...
int     printf_x_zeros(char *tmp, t_lists *list, int tmp_i, char *str)
{
    if (!((list->dot || list->dotzero) && !ft_strcmp(str, "0") && list->spec == 'x') || !list->sharp)
        while (*str)
            tmp[tmp_i++] = *str++;
//    else if (!((list->dot || list->dotzero) && !ft_strcmp(str, "0") && list->spec == 'x'))
//        while (*str)
//            tmp[tmp_i++] = *str++;
//    else
//        while (*str)
